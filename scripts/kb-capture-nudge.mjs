...
import { readHookInput, findBases, readConfig, readTranscript, userPrompts, filesWritten, wroteToBase, pendingNotes } from './lib/kb-hook-lib.mjs';

const FIRST_NUDGE_AT = 10; // user turns before the first reminder
const NUDGE_EVERY = 15;    // and its cadence afterwards
...
try {
  ...
  } else if (turns >= FIRST_NUDGE_AT && (turns - FIRST_NUDGE_AT) % NUDGE_EVERY === 0) {
    if (!wroteToBase(filesWritten(rows), bases)) {
